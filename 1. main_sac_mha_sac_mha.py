...
parser_sac_mha = argparse.ArgumentParser(description='PyTorch SAC-FF Args')
parser_sac_mha.add_argument('--policy', default="Gaussian",
                    help='Policy Type: Gaussian | Deterministic (default: Gaussian)')
parser_sac_mha.add_argument('--eval', type=bool, default=True,
                    help='Evaluates a policy a policy every 10 episode (default: True)')
parser_sac_mha.add_argument('--gamma', type=float, default=0.99, metavar='G',
                    help='discount factor for reward (default: 0.99)')
parser_sac_mha.add_argument('--tau', type=float, default=0.005, metavar='G',
                    help='target smoothing coefficient(τ) (default: 0.005)')
parser_sac_mha.add_argument('--lr', type=float, default=0.0003, metavar='G',
                    help='learning rate (default: 0.0003)')
parser_sac_mha.add_argument('--alpha', type=float, default=0.2, metavar='G',
                    help='Temperature parameter α determines the relative importance of the entropy\
                            term against the reward (default: 0.2)')
parser_sac_mha.add_argument('--adaptive_alpha', type=bool, default=True, metavar='G',
                    help='Automaically adjust α (default: True)')
parser_sac_mha.add_argument('--seed', type=int, default=123, metavar='N',
                    help='random seed (default: 123456)')
parser_sac_mha.add_argument('--batch_size', type=int, default=256, metavar='N',
                    help='batch size (default: 256)')
parser_sac_mha.add_argument('--num_episodes', type=int, default=1000001, metavar='N',
                    help='maximum number of training episodes (default: 1000000)')
parser_sac_mha.add_argument('--update_itr', type=int, default=2, metavar='N',
                    help='neural network updates frequency (default: 1)')
parser_sac_mha.add_argument('--target_update_interval', type=int, default=1, metavar='N',
                    help='Value target update per no. of actor updates  (default: 1)')
parser_sac_mha.add_argument('--replay_size', type=int, default=1000000, metavar='N',
                    help='size of replay buffer (default: 10000000)')
parser_sac_mha.add_argument('--state_dim', type = int, default = 7, metavar = 'N', 
                    help = 'dimension of state')
parser_sac_mha.add_argument('--action_dim', type = int, default = 1, metavar = 'N', 
                    help = 'dimension of action')
parser_sac_mha.add_argument('--hidden_dim', type = int, default = 256, metavar = 'N', 
                    help = 'number of units in hidden layer')
parser_sac_mha.add_argument('--num_heads', type = int, default = 2, metavar = 'N', 
                    help = 'number of heads in multihead attention layer')
parser_sac_mha.add_argument('--cuda_is_available', type = bool, default = True,
                    help = 'Gpu or cpu')
args_sac_mha = parser_sac_mha.parse_args()
...
sac_trainer1 = SAC_MHA(args_sac_mha)
sac_trainer2 = SAC_MHA(args_sac_mha)
#%%
...
if __name__ == '__main__':
        ...
        replay_buffer1 = ReplayBuffer_SAC(args_sac_mha.state_dim, args_sac_mha.action_dim, True)
        replay_buffer2 = ReplayBuffer_SAC(args_sac_mha.state_dim, args_sac_mha.action_dim, True)

        # training loop
        for eps in range(args_sac_mha.num_episodes):
            if eps % 50_000 == 0:
                print('**********************')
                print('episode', eps)
                print('**********************')
            
            # Randomly select an index between 0 and 319
            ...
            for step in range(24):
                ...
                action1 = sac_trainer1.select_action(torch.FloatTensor(hub1.state_price).unsqueeze(0).unsqueeze(0).to(device), deterministic = False, with_logprob=False)          
                hub1.retail_price_tanh = action1[0]
                
                action2 = sac_trainer2.select_action(torch.FloatTensor(hub2.state_price).unsqueeze(0).unsqueeze(0).to(device), deterministic = False, with_logprob=False)
                #action2 = action2[0][0][0].detach().numpy()
                hub2.retail_price_tanh = action2[0]
                    
                hub1.update_retail_price(env.list_da_prices[t], env.list_rt_prices[t])
                
                #print(hub1.retail_price)  
                ...
                if eps % 50_000 == 0:
                    ...
                    print('t:', t, 'DRL-output1',action1[0],'DRL-output2',action2[0] )
                ...
                if done:
                    break
            if eps > 15 :
                    for i in range(args_sac_mha.update_itr):
            ...
            if eps % 50 == 0 and eps>0: # plot and model saving interval
                ...
                np.save('./model/sac_mha_sac_mha/rewards1', rewards1)
                np.save('./model/sac_mha_sac_mha/rewards2', rewards2)
