...
parser_dqn_ff = argparse.ArgumentParser(description='PyTorch DQN-FF Args')
parser_dqn_ff.add_argument('--eps_start', type = float, default = 0.8, metavar = 'N', 
                    help = 'start value of epsilon')
parser_dqn_ff.add_argument('--eps_end', type = float, default = 0.05, metavar = 'N', 
                    help = 'end value of epsilon')
parser_dqn_ff.add_argument('--eps_decay', type = float, default = 0.8, metavar = 'N', 
                    help = 'decay rate of ep')
parser_dqn_ff.add_argument('--policy', default="Gaussian",
                    help='Policy Type: Gaussian | Deterministic (default: Gaussian)')
parser_dqn_ff.add_argument('--eval', type=bool, default=True,
                    help='Evaluates a policy a policy every 10 episode (default: True)')
parser_dqn_ff.add_argument('--gamma', type=float, default=0.99, metavar='G',
                    help='discount factor for reward (default: 0.99)')
parser_dqn_ff.add_argument('--target_update_interval', type=int, default=100, metavar='G',
                    help='target update interval (default: 100)')
parser_dqn_ff.add_argument('--lr', type=float, default=0.0003, metavar='G',
                    help='learning rate (default: 0.0003)')
parser_dqn_ff.add_argument('--seed', type=int, default=123, metavar='N',
                    help='random seed (default: 123456)')
parser_dqn_ff.add_argument('--batch_size', type=int, default=256, metavar='N',
                    help='batch size (default: 256)')
parser_dqn_ff.add_argument('--num_episodes', type=int, default=1000001, metavar='N',
                    help='maximum number of training episodes (default: 1000000)')
parser_dqn_ff.add_argument('--update_itr', type=int, default=2, metavar='N',
                    help='neural network updates frequency (default: 1)')
parser_dqn_ff.add_argument('--replay_size', type=int, default=1000000, metavar='N',
                    help='size of replay buffer (default: 10000000)')
parser_dqn_ff.add_argument('--state_dim', type = int, default = 7, metavar = 'N', 
                    help = 'dimension of state')
parser_dqn_ff.add_argument('--action_dim', type = int, default = 101, metavar = 'N', 
                    help = 'dimension of action')
parser_dqn_ff.add_argument('--hidden_dim', type = int, default = 256, metavar = 'N', 
                    help = 'number of units in hidden layer')
parser_dqn_ff.add_argument('--cuda_is_available', type = bool, default = True,
                    help = 'Gpu or cpu')
args_dqn_ff = parser_dqn_ff.parse_args()
...
dqn_trainer1 = DQN(args_dqn_ff)
dqn_trainer2 = DQN(args_dqn_ff)     
#%%
...
if __name__ == '__main__':   
    ...
    replay_buffer1 = replay_buffer_DQN(args_dqn_ff.replay_size)
    r_buffer2 = replay_buffer_DQN(args_dqn_ff.replay_size)
    
    
    for eps in range(args_dqn_ff.num_episodes):
        if eps % 50_000 == 0:
                print('**********************')
                print('episode', eps)
                print('**********************')
        ...
        for step in range(24):
            ...
            replay_buffer1.add([hub1.state_price, hub1.next_state_price, [action1], [hub1.reward_pricing], [done]])
            ...
            if eps % 50_000 == 0:
                    ...
                    print('epsilon', dqn_trainer2.epsilon_scheduler.epsilon)
            if done:
                    break
                
        if eps > 15:
            for i in range(args_dqn_ff.update_itr):
                sample1 = replay_buffer1.sample(args_dqn_ff.batch_size)
                sample2 = r_buffer2.sample(args_dqn_ff.batch_size)
        ...
        if eps % 50 == 0 and eps>0: # plot and model saving interval
                ...
                np.save('./model/dqn_ff_dqn_ff/rewards1', rewards1)
                np.save('./model/dqn_ff_dqn_ff/rewards2', rewards2)
