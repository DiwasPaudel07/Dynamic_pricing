# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 16:30:03 2023

@author: diwaspaudel
"""

import torch
from common.actor_networks import Actor_FF
from common.critic_networks import Critic_FF
...
class SAC_FF(object):
    def __init__(
        ...
        self.actor = Actor_FF(self.state_dim, self.action_dim, self.hidden_dim, self.device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr = self.lr)
        self.q1 = Critic_FF(self.state_dim, self.action_dim, self.hidden_dim, self.device)
        self.q1_optimizer = torch.optim.Adam(self.q1.parameters(), lr = self.lr)
        self.q2 = Critic_FF(self.state_dim, self.action_dim, self.hidden_dim, self.device)
    ...
    def save_model1(self, episode):
            torch.save(self.q1.state_dict(), "./model/agent1_sac_ff_q1_{}.pth".format(episode))
            torch.save(self.q2.state_dict(), "./model/agent1_sac_ff_q2_{}.pth".format(episode))
            torch.save(self.actor.state_dict(), "./model/agent1_sac_ff_policy_{}.pth".format(episode))
            
            
    def save_model2(self, episode):
            torch.save(self.q1.state_dict(), "./model/agent2_sac_ff_q1_{}.pth".format(episode))
            torch.save(self.q2.state_dict(), "./model/agent2_sac_ff_q2_{}.pth".format(episode))
            torch.save(self.actor.state_dict(), "./model/agent2_sac_ff_policy_{}.pth".format(episode))